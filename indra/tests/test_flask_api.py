...
def test_filter_grounded_only_score():
    ...
    st1 = Influence(c1, c2)
    assert len(_post_stmts_preassembly([st1], route)) == 1
    assert len(_post_stmts_preassembly([st1], route, score_threshold=0.4)) == 1
    assert len(_post_stmts_preassembly([st1], route, score_threshold=0.6)) == 1
    assert len(_post_stmts_preassembly([st1], route, score_threshold=0.85)) == 0
    assert len(_post_stmts_preassembly([st1], route, score_threshold=0.95)) == 0
...
def test_filter_belief():
    ...
    st_out = _post_stmts_preassembly([st1, st2, st3], route, belief_cutoff=0.75)
    assert len(st_out) == 2
