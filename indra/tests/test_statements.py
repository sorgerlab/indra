...
def test_refinement_agent_mod_order():
    ...
    hras2 = Agent('MAP2K1',
        mods=[ModCondition('ubiquitination'), ModCondition('phosphorylation')])
    assert(hras1.refinement_of(hras2, eh, mh, ch))
    assert(not hras2.refinement_of(hras1, eh, mh, ch))
...
def test_refinement_agent_mod_multiple():
    ...
    assert(st3.refinement_of(st1, eh, mh, ch))
...
def test_refinement_agent_mod_generic():
    ...
    assert(raf3p.refinement_of(raf2p, eh, mh, ch))
    assert(not raf2p.refinement_of(raf3p, eh, mh, ch))
...
def test_agent_superfamily_refinement():
    ...
    assert nras.refinement_of(ras, eh, mh, ch)
    assert not ras.refinement_of(nras, eh, mh, ch)
...
def test_agent_boundcondition_refinement():
    ...
    # nras1 (bound to BRAF)
    assert not nras2.refinement_of(nras1, eh, mh, ch)
    assert not nras3.refinement_of(nras1, eh, mh, ch)
    assert not nras4.refinement_of(nras1, eh, mh, ch)
    assert not nras5.refinement_of(nras1, eh, mh, ch)
    # nras2 (bound to CRAF)
    assert not nras1.refinement_of(nras2, eh, mh, ch)
    assert not nras3.refinement_of(nras2, eh, mh, ch) # Not bound condition
    assert not nras4.refinement_of(nras2, eh, mh, ch)
    assert not nras5.refinement_of(nras2, eh, mh, ch)
    # nras3 (not bound to CRAF)
    assert not nras1.refinement_of(nras3, eh, mh, ch)
    assert not nras2.refinement_of(nras3, eh, mh, ch) # Not bound condition
    assert not nras4.refinement_of(nras3, eh, mh, ch)
    assert not nras5.refinement_of(nras3, eh, mh, ch)
    # nras4 (no bound condition)
    assert nras1.refinement_of(nras4, eh, mh, ch)
    assert nras2.refinement_of(nras4, eh, mh, ch)
    assert nras3.refinement_of(nras4, eh, mh, ch)
    assert nras5.refinement_of(nras4, eh, mh, ch)
    # nras5 (RAF family bound condition)
    assert nras1.refinement_of(nras5, eh, mh, ch)
    assert nras2.refinement_of(nras5, eh, mh, ch)
    assert not nras3.refinement_of(nras5, eh, mh, ch)
    assert not nras4.refinement_of(nras5, eh, mh, ch)
...
def test_agent_modification_refinement():
    ...
    assert mek2.refinement_of(mek1, eh, mh, ch)
    assert mek3.refinement_of(mek1, eh, mh, ch)
    assert mek4.refinement_of(mek1, eh, mh, ch)
    assert mek5.refinement_of(mek1, eh, mh, ch)
    assert mek6.refinement_of(mek1, eh, mh, ch)
    assert mek7.refinement_of(mek1, eh, mh, ch)
    assert mek8.refinement_of(mek1, eh, mh, ch)
    # mek2
    assert not mek1.refinement_of(mek2, eh, mh, ch)
    assert not mek3.refinement_of(mek2, eh, mh, ch) # Different site
    assert mek4.refinement_of(mek2, eh, mh, ch)
    assert not mek5.refinement_of(mek2, eh, mh, ch) # Cross-relationship
    assert mek6.refinement_of(mek2, eh, mh, ch)
    assert not mek7.refinement_of(mek2, eh, mh, ch) # Different site
    assert mek8.refinement_of(mek2, eh, mh, ch)
    # mek3
    assert not mek1.refinement_of(mek3, eh, mh, ch)
    assert not mek2.refinement_of(mek3, eh, mh, ch)
    assert mek4.refinement_of(mek3, eh, mh, ch)
    assert not mek5.refinement_of(mek3, eh, mh, ch)
    assert not mek6.refinement_of(mek3, eh, mh, ch)
    assert mek7.refinement_of(mek3, eh, mh, ch)
    assert mek8.refinement_of(mek3, eh, mh, ch)
    # mek4
    assert not mek1.refinement_of(mek4, eh, mh, ch)
    assert not mek2.refinement_of(mek4, eh, mh, ch)
    assert not mek3.refinement_of(mek4, eh, mh, ch)
    assert not mek5.refinement_of(mek4, eh, mh, ch)
    assert not mek6.refinement_of(mek4, eh, mh, ch)
    assert not mek7.refinement_of(mek4, eh, mh, ch)
    assert mek8.refinement_of(mek4, eh, mh, ch)
    # mek5
    assert not mek1.refinement_of(mek5, eh, mh, ch)
    assert not mek2.refinement_of(mek5, eh, mh, ch)
    assert not mek3.refinement_of(mek5, eh, mh, ch)
    assert not mek4.refinement_of(mek5, eh, mh, ch)
    assert mek6.refinement_of(mek5, eh, mh, ch)
    assert mek7.refinement_of(mek5, eh, mh, ch)
    assert mek8.refinement_of(mek5, eh, mh, ch)
    # mek6
    assert not mek1.refinement_of(mek6, eh, mh, ch)
    assert not mek2.refinement_of(mek6, eh, mh, ch)
    assert not mek3.refinement_of(mek6, eh, mh, ch)
    assert not mek4.refinement_of(mek6, eh, mh, ch)
    assert not mek5.refinement_of(mek6, eh, mh, ch)
    assert not mek7.refinement_of(mek6, eh, mh, ch)
    assert mek8.refinement_of(mek6, eh, mh, ch)
    # mek7
    assert not mek1.refinement_of(mek7, eh, mh, ch)
    assert not mek2.refinement_of(mek7, eh, mh, ch)
    assert not mek3.refinement_of(mek7, eh, mh, ch)
    assert not mek4.refinement_of(mek7, eh, mh, ch)
    assert not mek5.refinement_of(mek7, eh, mh, ch)
    assert not mek6.refinement_of(mek7, eh, mh, ch)
    assert mek8.refinement_of(mek7, eh, mh, ch)
    # mek8
    assert not mek1.refinement_of(mek8, eh, mh, ch)
    assert not mek2.refinement_of(mek8, eh, mh, ch)
    assert not mek3.refinement_of(mek8, eh, mh, ch)
    assert not mek4.refinement_of(mek8, eh, mh, ch)
    assert not mek5.refinement_of(mek8, eh, mh, ch)
    assert not mek6.refinement_of(mek8, eh, mh, ch)
    assert not mek7.refinement_of(mek8, eh, mh, ch)
...
def test_phosphorylation_modification_refinement():
    ...
    # p1
    assert p2.refinement_of(p1, eh, mh, ch)
    assert p3.refinement_of(p1, eh, mh, ch)
    assert p4.refinement_of(p1, eh, mh, ch)
    assert p5.refinement_of(p1, eh, mh, ch)
    assert p6.refinement_of(p1, eh, mh, ch)
    # p2
    assert not p1.refinement_of(p2, eh, mh, ch)
    assert not p3.refinement_of(p2, eh, mh, ch)
    assert not p4.refinement_of(p2, eh, mh, ch)
    assert p5.refinement_of(p2, eh, mh, ch)
    assert not p6.refinement_of(p2, eh, mh, ch)
    # p3
    assert not p1.refinement_of(p3, eh, mh, ch)
    assert not p2.refinement_of(p3, eh, mh, ch)
    assert not p4.refinement_of(p3, eh, mh, ch)
    assert not p5.refinement_of(p3, eh, mh, ch)
    assert p6.refinement_of(p3, eh, mh, ch)
    # p4
    assert not p1.refinement_of(p4, eh, mh, ch)
    assert not p2.refinement_of(p4, eh, mh, ch)
    assert not p3.refinement_of(p4, eh, mh, ch)
    assert p5.refinement_of(p4, eh, mh, ch)
    assert p6.refinement_of(p4, eh, mh, ch)
    # p5
    assert not p1.refinement_of(p5, eh, mh, ch)
    assert not p2.refinement_of(p5, eh, mh, ch)
    assert not p3.refinement_of(p5, eh, mh, ch)
    assert not p4.refinement_of(p5, eh, mh, ch)
    assert not p6.refinement_of(p5, eh, mh, ch)
    # p6
    assert not p1.refinement_of(p6, eh, mh, ch)
    assert not p2.refinement_of(p6, eh, mh, ch)
    assert not p3.refinement_of(p6, eh, mh, ch)
    assert not p4.refinement_of(p6, eh, mh, ch)
    assert not p5.refinement_of(p6, eh, mh, ch)
...
def test_activitymod_refinement():
    ...
    assert p5.refinement_of(p1, eh, mh, ch)
    assert p6.refinement_of(p1, eh, mh, ch)
    assert p7.refinement_of(p1, eh, mh, ch)
    # p2
    assert not p1.refinement_of(p2, eh, mh, ch)
    assert not p3.refinement_of(p2, eh, mh, ch)
    assert not p4.refinement_of(p2, eh, mh, ch)
    assert p5.refinement_of(p2, eh, mh, ch)
    assert not p6.refinement_of(p2, eh, mh, ch)
    assert p7.refinement_of(p2, eh, mh, ch)
    # p3
    assert not p1.refinement_of(p3, eh, mh, ch)
    assert not p2.refinement_of(p3, eh, mh, ch)
    assert p4.refinement_of(p3, eh, mh, ch)
    assert p5.refinement_of(p3, eh, mh, ch)
    assert p6.refinement_of(p3, eh, mh, ch)
    assert p7.refinement_of(p3, eh, mh, ch)
    # p4
    assert not p1.refinement_of(p4, eh, mh, ch)
    assert not p2.refinement_of(p4, eh, mh, ch)
    assert not p3.refinement_of(p4, eh, mh, ch)
    assert p5.refinement_of(p4, eh, mh, ch)
    assert p6.refinement_of(p4, eh, mh, ch)
    assert p7.refinement_of(p4, eh, mh, ch)
    # p5
    assert not p1.refinement_of(p5, eh, mh, ch)
    assert not p2.refinement_of(p5, eh, mh, ch)
    assert not p3.refinement_of(p5, eh, mh, ch)
    assert not p4.refinement_of(p5, eh, mh, ch)
    assert not p6.refinement_of(p5, eh, mh, ch)
    assert p7.refinement_of(p5, eh, mh, ch)
    # p6
    assert not p1.refinement_of(p6, eh, mh, ch)
    assert not p2.refinement_of(p6, eh, mh, ch)
    assert not p3.refinement_of(p6, eh, mh, ch)
    assert not p4.refinement_of(p6, eh, mh, ch)
    assert not p5.refinement_of(p6, eh, mh, ch)
    assert p7.refinement_of(p6, eh, mh, ch)
    # p7
    assert not p1.refinement_of(p7, eh, mh, ch)
    assert not p2.refinement_of(p7, eh, mh, ch)
    assert not p3.refinement_of(p7, eh, mh, ch)
    assert not p4.refinement_of(p7, eh, mh, ch)
    assert not p5.refinement_of(p7, eh, mh, ch)
    assert not p6.refinement_of(p7, eh, mh, ch)
...
def test_mismatched_complex_refinement():
    ...
    assert not st1.refinement_of(st2, eh, mh, ch)
    assert not st2.refinement_of(st1, eh, mh, ch)
...
def test_location_refinement():
    ...
    assert(a4.refinement_of(a5, eh, mh, ch))
