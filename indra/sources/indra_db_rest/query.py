...
Broadly, querie classes can be broken into 3 types: queries on
...
instead make use of the overloaded ``&`` and ``|`` operators put Queries
together into more complex structures. In addition you can invert a query
-- essentially ask for Statements that do *not* meet certain criteria, e.g.
"not has readings". This can be accomplished with the overloaded ``~`` operator,
e.g. ``~HasReadings()``.

The query class works by representing and producing a particular JSON
...
:py:func:`get_statements_from_query <indra.sources.indra_db_rest.api.get_statements_from_query>`
