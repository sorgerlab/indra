...
class SofiaJsonProcessor(object):
    def __init__(self, results_json):
        self.statements = []
        self._events = self._process_events(results_json)
        self.statements = self._process_relations(results_json, self._events)

    @staticmethod
    def _process_events(json):
        event_dict = {}
        for _dict in json:
            events = _dict['Events']
            for event in events:
    ...
    @staticmethod
    def _process_relations(json, event_dict):
        stmts = []
        for _dict in json:
            json_relations = _dict['Causal']
            for rel_dict in json_relations:
...
class SofiaExcelProcessor(object):
