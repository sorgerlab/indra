...
class WormBaseProcessor(object):
    ...
    def __init__(self, data, mappings_df):
        ...
        self.wb_to_entrez_dict = self.mappings_df.set_index('wormbase_id')['GeneID'].to_dict()
        self.entrez_to_wb_dict = self.mappings_df.set_index('GeneID')['wormbase_id'].to_dict()
        self.entrez_to_symbol_dict = self.mappings_df.set_index('GeneID')['Symbol'].to_dict()
        self.symbol_to_annotation_dict = self.mappings_df.set_index('GeneID').to_dict(orient='index')
    ...
    def process_row(self, wb_row):
        ...
        if not db_id_info_agent_a:
            ...
            if not wormbase_id_agent_a and entrez_id_agent_a:  # If WB ID isn't found but an entrez ID is,
                wormbase_id_agent_a = self.entrez_to_wb_dict.get(entrez_id_agent_a) or None
        ...
        if not db_id_info_agent_b:
            ...
            if not wormbase_id_agent_b and entrez_id_agent_b:  # If WB ID isn't found but an entrez ID is,
                wormbase_id_agent_b = self.entrez_to_wb_dict.get(entrez_id_agent_b) or None
        ...
        if entrez_id_agent_a:
            entrez_name_agent_a = self.entrez_to_symbol_dict.get(entrez_id_agent_a) or None
        ...
        if entrez_id_agent_b:
            entrez_name_agent_b = self.entrez_to_symbol_dict.get(entrez_id_agent_b) or None
        ...
        if entrez_id_agent_a:
            full_annotations['entrez_info_agent_a'] = self.symbol_to_annotation_dict.get(entrez_id_agent_a) or {}
        if entrez_id_agent_b:
            full_annotations['entrez_info_agent_b'] = self.symbol_to_annotation_dict.get(entrez_id_agent_b) or {}
