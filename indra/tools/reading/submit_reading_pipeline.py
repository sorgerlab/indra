...
usage = 'Usage: %s pmid_list tmp_dir num_nodes num_cores_per_node' % sys.argv[0]
if len(sys.argv) != 5:
    print usage
    sys.exit(1)
...
# First, figure out how many papers there are by counting the number of lines
# in the file.
...
# Next, submit jobs for offline reading with REACH. This will involve running
# num_nodes jobs, each with num_cores assigned to each. The papers are divided
# up into chunks such that all the papers will be processed in a single round.
...
for node_start_ix in node_start_pts:
    ...
    cmd_list = ['qsub', '-b', 'y', '-V', '-cwd', '-N', REACH_JOB_NAME, '-pe',
                'orte', str(num_cores), 'python', 'run_reach_on_pmids.py',
                pmid_list, tmp_dir, str(num_cores), str(node_start_ix),
                str(node_end_ix)]
    print ' '.join(cmd_list)
    subprocess.call(cmd_list)

# Next, submit jobs for processing the REACH statements with INDRA. These will
# be parallelized across cores rather than nodes, so we divide the jobs
# accordingly. We make the start of these jobs contingent on the completion
# of all of the REACH jobs.
