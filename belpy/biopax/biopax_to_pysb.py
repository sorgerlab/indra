...
def get_create_monomer(model,biopax_element):
    ...
    # Call recursively for each element of a complex
        ...
        for e in biopax_element.getMemberPhysicalEntity().toArray():
...
def get_reaction_players(reaction):
    ...
    return (left,right,catalyzers,controllers)
...
def assemble(model,reaction):
    left,right,catalyzers,controllers = get_reaction_players(reaction)
    if len(left)!=1:
    ...
    # Break it up for one rule per catalyzer/controller
    for c in catalyzers+controllers:
        if iscomplex(c):
            warnings.warn('Cannot handle complexes.')
            continue
